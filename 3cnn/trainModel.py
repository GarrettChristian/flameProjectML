...
def getModel():
    ...
    model.add(layers.Conv2D(filters=32, kernel_size=5, strides=1, activation='relu', padding='same')) 
...
def main():
    ...
    train_data, test_data, _, _ = train_test_split(
        files, labelsTmp, test_size=0.3, random_state=21
    )
    ...
    history = model.fit(training_generator, validation_data=validation_generator, epochs=25)
